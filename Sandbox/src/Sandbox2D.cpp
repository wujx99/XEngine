...
void Sandbox2D::OnAttach()
{
	...
	XEg::FramebufferSpecification fbSpec;
	fbSpec.Width = 1280;
	fbSpec.Height = 720;
	m_Framebuffer = XEg::Framebuffer::Create(fbSpec);
	...
	if (dockingEnabled)
	{
		...
		if (ImGui::BeginMenuBar())
		{
			if (ImGui::BeginMenu("File"))
			{
				...
				if (ImGui::MenuItem("Exit")) XEg::Application::Get().Close();
				ImGui::EndMenu();
			}
			ImGui::EndMenuBar();
		}
		...
		uint32_t textureID2 = m_CheckerboardTexture->GetRendererID();
		ImGui::Image((void*)textureID2, ImVec2{ 1280, 720 });
		ImGui::End();
