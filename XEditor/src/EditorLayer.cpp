...
namespace XEg
{
	...
	void EditorLayer::OnUpdate(TimeStep ts)
	{
		...
		// Resize by imgui viewport panel
		if (XEg::FramebufferSpecification spec = m_Framebuffer->GetSpecification();
		...
		if (mouseX >= 0 && mouseY >= 0 && mouseX < (int)viewportSize.x && mouseY < (int)viewportSize.y)
		{
			...
			XE_CORE_WARN("pixeldata: {0}", pixelData);
	...
	void EditorLayer::OnImGuiRender()
	{
		...
		style.WindowMinSize.x = 370; // set the minWindow width of dockspace
		...
		style.WindowMinSize.x = minWinSizeX; // restore
		...
		if (m_HoveredEntity)
			name = m_HoveredEntity.GetComponent<TagComponnent>().Tag;
		...
		//XE_CLIENT_WARN("Viewport Size: {0}, {1}", viewportPanelSize.x, viewportPanelSize.y);
	...
	{
		std::string filepath = FileDialogs::OpenFile("XEngine Scene(*.XE)\0*.XE\0");
		if (!filepath.empty())
		{
			OpenScene(filepath);
		}
	...
	{
		if (path.extension().string() != ".XE")
		{
			XE_CLIENT_WARN("Could not load {0} - not a scene file", path.filename().string());
			return;
		}
	...
	{
		std::string filepath = FileDialogs::SaveFile("XEngine Scene (*.XE)\0*.XE\0");
		if (!filepath.empty())
		{
			SceneSerializer serializer(m_ActiveScene);
